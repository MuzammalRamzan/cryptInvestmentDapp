...
interface ModalClaimInfoProps {
  isOpen: boolean;
  onClose: () => void;
}

const ModalClaimInfo: React.FC<ModalClaimInfoProps> = ({ isOpen, onClose }) => {
  return isOpen ? (
        ...
        <div className="max-h-screen w-[95vw] md:w-[90vw] lg:w-[60vw] xl:w-[40vw] min-h-[15rem] relative border-0 rounded-2xl shadow-lg flex flex-col justify-between bg-white outline-none focus:outline-none overflow-y-auto overflow-x-hidden text-sm lg-text-lg py-4 pt-0 px-6 md:px-10 ">
          ...
          <div className="flex-1 w-full h-fit mt-4">
            <div className="w-full  mt-[24px] mb-[32px]">
              <p>
                Your purchased tokens will be allocated to vesting smart
                contract upon purchase, and will not available to claim to your
                wallet just yet, You will be able to claim your available tokens
                during exchange listing event. Tokens will be listed after the
                end of all presale rounds. Estimated end date of each presale is
                mentioned on the buy widget itself.
              </p>
            </div>
          </div>
        </div>
