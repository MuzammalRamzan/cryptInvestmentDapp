...
interface IPartialPaymentProps {
  amountUSD: number;
  chainType: string;
  YPREDAmount: number | string;
  qrAddress: string;
  balancePaid: number;
  onChangeTimeout: () => void;
  amountByChain: number;
...
const PartialPayment = ({
  ...
  const amountToBePaid =
    chainType === "usdt"
      ? amountUSD - balancePaid
      : amountByChain - balancePaid;

  return (
    <>
      <div className="py-4 pt-0 px-6 md:px-10 flex  flex-column justify-center flex-wrap space-y-0 md:space-y-2 space-x-1 md:space-x-6 items-center">
        <div
        ...
        <div className="min-w-[98px]">
          ...
          <div className="flex items-center space-x-1 border p-2 rounded-xl">
            ...
            <input
              ...
              readOnly
              onChange={(e) => {
                const value = (e.target.value || "0").replace(/\D/g, "");
                onChangeAmountUSD(value);
                if (Number(value) < MIN_USD) {
                  toast.dismiss();
      ...
      <div className="flex flex-wrap border-t-2 items-center justify-center w-full py-2 gap-5">
      ...
      <div className="relative flex flex-col items-center border-t-2 w-[80%] self-center flex-1 pt-2">
        <div className="text-lg -tracking-tighter mt-4 flex flex-row space-x-2">
          <p>PLEASE PAY REMAINING</p>
          <p className="font-semibold text-[#ed3a85]">{`${
            Math.ceil(amountToBePaid * 1000) / 1000
          }  ${
            chainType === "bsc"
              ? "BNB"
              : chainType === "polygon"
        ...
        <div className="text-[#bdbdbd] text-[14px] font-medium">
          Your transaction is still pending. Make payment before time out!
        </div>
        {
          <div className="w-[30%] my-2 min-w-[200px] flex justify-center items-center">
            {qrAddress &&
            ((amountToBePaid > 0 && chainType !== "usdt") ||
              (chainType === "usdt" && amountToBePaid > 0 && qrAddress)) ? (
              <img
                src={`https://chart.googleapis.com/chart?cht=qr&chl=ethereum%3A${qrAddress}%3Famount%3D${amountToBePaid}&chs=150x150&choe=UTF-8&chld=L%7C2`}
        ...
        <div className="flex items-center space-x-2">
          <div>
            {amountUSD >= MIN_USD ? truncateHash(qrAddress, 5) : "...."}
          </div>
          <button
